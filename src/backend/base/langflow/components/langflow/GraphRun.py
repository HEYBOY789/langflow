from typing import Any, Union  # noqa: N999

from langgraph.graph import END
from langgraph.store.postgres.aio import AsyncPostgresStore

from langflow.custom import Component
from langflow.io import BoolInput, DictInput, HandleInput, IntInput, MessageTextInput, Output
...
class GraphRun(Component):
    ...
    async def _execute_graph_async(self) -> dict[str, Any]:
        """Helper method to execute the graph asynchronously."""
        try:
            self.serialize_input()
            self.normalize_input_data()
            print("####################")  # noqa: T201
            print(self.config)  # noqa: T201
            print("####################")  # noqa: T201
            result = await self._graph.ainvoke(input=self.input_data, config=self.config)
        except (ValueError, TypeError, KeyError, AttributeError) as e:
            msg = f"Check the input state: {e}"
            raise ValueError(msg) from e
        else:
            if result is None:
                return {"success": True, "result": None, "message": "Output state is not set."}
    ...
    def _parse_class_instance(self, value: str, variables: dict):
        ...
        # Replace nested class variables
        ...
        if params_str:
            import json
            try:
                params = json.loads(f"{params_str}")  # Wrap in braces to make valid JSON
    ...
    def _replace_variables_in_json(self, json_str: str, variables: dict) -> str:
        ...
        for i, part in enumerate(parts):
            ...
            # Inside strings (i % 2 == 1), keep as is
