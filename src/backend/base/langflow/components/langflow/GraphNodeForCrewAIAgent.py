...
class GraphNodeForCrewAIAgent(Component):
    ...
    outputs = [
        Output(display_name="Next Node", name="next_node", method="build_graph")
    ]
    ...
    async def run_crewai_agent(self, state, _original_prompts) -> dict[str, Any]:
        ...
        # Format role
        formatted_role = self.form_extraction_system_prompt(role)
        formatted_role = formatted_role.replace("{langflow_mem_data}", mem_str)
        formatted_role = self.format_prompt(formatted_role, state)

        # Format goal
        formatted_goal = self.form_extraction_system_prompt(goal)
        formatted_goal = formatted_goal.replace("{langflow_mem_data}", mem_str)
        formatted_goal = self.format_prompt(formatted_goal, state)

        # Format backstory
        formatted_backstory = self.form_extraction_system_prompt(backstory)
        formatted_backstory = formatted_backstory.replace("{langflow_mem_data}", mem_str)
        formatted_backstory = self.format_prompt(formatted_backstory, state)
        ...
        formatted_prompt = self.format_prompt(formatted_prompt, state)
    ...
    def build_graph(self) -> "GraphNodeForCrewAIAgent":
        ...
        async def node_function(  # noqa: D417
            ...
            try:
                ...
                # Get result
                result = await self.run_crewai_agent(state, original_prompts)
    ...
    def _clarify_result(self, result):
        ...
        return self._parse_json_from_text(
            str(result.raw)) if hasattr(result, "raw") else self._parse_json_from_text(str(result)
                                                                                                )
