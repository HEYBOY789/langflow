...
class GraphNodeForCrewAICrew(Component):
    ...
    async def run_crew(self, state) -> dict[str, Any]:
        ...
        # Create a local copy of state to avoid modifying the shared object
        inputs__ = state.__dict__.copy()  # Create a copy of the dict representation

        # Add model instruction to state
        verbose_str = self.output_state.schema.get("verbose_schema_str")
        inputs__["langflow_model_schema"] = verbose_str

        # Add memories to inputs if any
        mem_str = self.form_memory_str_for_prompt() if self.memories else ""
        if mem_str:
            inputs__["langflow_mem_data"] = mem_str

        print("debug inputs to crew:", inputs__)  # noqa: T201

        result = await self.crew.kickoff_async(inputs=inputs__)
    ...
    def build_graph(self) -> "GraphNodeForCrewAICrew":
        ...
        async def node_function(  # noqa: D417
                state: self.input_model, store: BaseStore | None=None, config:RunnableConfig | None=None # type: ignore  # noqa: PGH003
            ...
            try:
                ...
                # Get result
                result = await self.run_crew(state)
