...
class GraphNodeForAgent(Component):
    ...
    async def run_agent(self, state, _original_prompts) -> dict[str, Any]:
        ...
        formatted_system_prompt = self.form_extraction_system_prompt(original_system_prompt)
        formatted_system_prompt = formatted_system_prompt.replace("{langflow_mem_data}", mem_str)
        formatted_system_prompt = self.format_prompt(formatted_system_prompt, state)

        # ALWAYS use the original template, never the current agent component text\
        formatted_input_text = self.form_extraction_system_prompt(original_input_template)
        formatted_input_text = formatted_input_text.replace("{langflow_mem_data}", mem_str)
        formatted_input_text = self.format_prompt(formatted_input_text, state)
    ...
    def build_graph(self) -> "GraphNodeForAgent":
        ...
        async def node_function(state: self.input_model, store: BaseStore | None=None, config:RunnableConfig | None=None) -> Literal[self.output_model, command_type_hint]: # type: ignore  # noqa: D417, E501, PGH003
            ...
            try:
                ...
                # Get result
                result = await self.run_agent(state, original_prompts)
