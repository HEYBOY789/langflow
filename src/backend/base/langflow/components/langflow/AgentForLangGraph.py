...
class AgentComponentForLangGraph(ToolCallingAgentComponent):
    ...
    description: str = (
        "Define the agent's instructions, then enter a task to complete using tools. For the prompt, use "
        "{variable} to refer to the state's fields. The 'variable' should match the fields in the "
        ...
        *openai_inputs_filtered,
        MultilineInput(
            ...
            value=(
                ...
                "{@model_schema}. Output normal message, no json, no code, the pydantic format is only for reference."
            ),
            advanced=False,
        ),
        IntInput(
